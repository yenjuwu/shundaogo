...
function azexo_purchase_url() {
    return 'http://codecanyon.net/item/item/' . AZEXO_ENVATO_ID . '?license=regular&open_purchase_for_item_id=' . AZEXO_ENVATO_ID . '&purchasable=source';
}
...
function azexo_oauth_login_callback() {
    ...
    if (isset($option['items']) && !azexo_is_activated()) {
        ?>
        <a href="<?php echo esc_url(azexo_purchase_url()); ?>" class="button button-primary" target="_blank"><?php echo esc_html_e('Purchase license of AZEXO HTML Customizer to activate it', 'azh'); ?></a>
        <?php
    } else {
        ?>
        <div class="oauth-login" data-username="<?php echo esc_attr(azexo_envato_username()); ?>">
            <a href="<?php echo esc_url(azexo_get_oauth_login_url(admin_url('admin.php?page=' . azexo_updater_option()))); ?>"
               class="oauth-login-button button button-primary"><?php echo esc_html_e('Activate AZEXO HTML Customizer', 'azh'); ?></a>
        </div>
        <?php
    }
}
