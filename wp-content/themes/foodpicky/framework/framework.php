...
define('AZEXO_FRAMEWORK_VERSION', '1.23');
...
function azexo_scripts() {
    ...
    if (in_array('administrator', (array) $user->roles)) {
        ...
        $fontend_variables['edit_button'] = esc_html__('Edit', 'foodpicky');
...
function azexo_entry_field($name) {
    ...
    switch ($name) {
        ...
        case 'post_comments_count':
            ...
            $comments = '<a href="' . esc_url(get_comments_link()) . '"><span class="count">' . $comment_count['total_comments'] . '</span><span class="label">' . esc_html__('comments', 'foodpicky') . '</span></a>';
    ...
    if (!empty($date_to)) {
        ...
        <div class="time-left">
            ...
            <div class="time" data-time="<?php print date('Y/m/d H:i:s', $date_to); ?>">
                <div class="days"><span class="count"><?php print $days; ?></span><span class="title"><?php print esc_html__('day', 'foodpicky'); ?></span></div>
                <div class="hours"><span class="count"><?php print $hours; ?></span><span class="title"><?php print esc_html__('hrs', 'foodpicky'); ?></span></div>
...
function azexo_build_vendor_info($post){
    if($post->post_type!="azl_profile"){
        return "";
    }
    $meta_value = get_post_meta($post->ID);
    $vendor_image = $meta_value['vendor_image'][0];
    $imgStrip="";
    if($vendor_image!="" && $vendor_image !=NULL){
        $imgStrip = "style=\"background-image:url('".$vendor_image."')\";";
    }
    
    $html ="<div id='middle' class='sidebar-container' role='complementary'>";
    $html.="<div class='sidebar-inner'>";
    $html.="<div data-section='section' class='static-header' ".$imgStrip." >";
        ...
        if (!empty($filter)) {
            ...
            if (is_array($filter_terms)) {
                $filter_terms = array_map(function($term) {
                    return base64_encode($term->slug);
                }, $filter_terms);
